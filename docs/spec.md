...
本文件每一條需求都標了來源，方便你在面試複盤時知道哪些是題目要的、哪些是我補的設計：

| 標記 | 意義 |
|---|---|
| 📄 | 原文件明確要求 |
| ⭐ | 原文件列為加分題 |
| 🧩 | **文件沒寫，我判斷應該補上的設計考量**（每條都附「為什麼」） |
| ❓ | 待你決定的開放項目 |
...
> 面試講法：「文件的 payload 沒有版序，我在 mock 層補了單調序號，因為一旦有斷線重連或多來源合併，亂序覆蓋是必然會發生的 bug，而且它不會 crash、只會讓畫面顯示錯的賠率 — 屬於最難被發現的那一類。」
...
- 提供 **debug 控制面板**（長按導覽列標題開啟）可即時把頻率調成 10 / 100 / 1000 筆每秒。
...
3. **更新合併（Coalescing）** — 🧩 文件沒提，但這是「畫面保持順暢」的真正解法。
   ...
   > 面試講法：「UI 更新頻率應該由畫面的刷新節奏決定，不該由資料的到達節奏決定。」
...
文件原文：「切換畫面後能快速恢復顯示」。🧩 我認為這句話涵蓋兩種情境，兩種都做：
...
> 🧩 **FR-6.4 是我最想強調的一條，文件完全沒提**：斷線期間的推播是**永久遺失**的，只重連不對帳，畫面會停在斷線前的舊賠率且**永遠不會自己修正**（除非那場剛好又被推播到）。這個 bug 不會 crash、測不出來、只會讓資料默默錯下去。
...
| # | 問題 | 狀態 |
|---|---|---|
| 1 | 模組化方案 | ✅ **已定案：本地 SPM package**（§5.1）|
| 2 | SnapKit 可否使用 | ✅ **已定案：可用，僅限 `OddsUI`**（§2）|
| 3 | 最低支援版本 | ✅ **已定案：iOS 16.0** |
| 4 | 詳情頁（FR-7）要做嗎？ | 預設要做（否則快取加分題無從展示）|
| 5 | Debug HUD / 加壓面板要做嗎？ | 預設要做（它是 FR-3.3 最有力的證據）|
| 6 | 專案名稱 | 預設 `OddsBoard`，待確認 |
| 7 | Repo 要 public 還是 private + 邀請？ | 依對方指示；預設 public |

### 開工前置作業（由使用者執行）

Xcode 專案由人工建立，避免手刻 `project.pbxproj`。設定：
Template `iOS → App`｜Interface **Storyboard**（建立後刪除 storyboard，改手動啟動 UIWindow；選 SwiftUI 會生成 SwiftUI entry point，不可用）｜Language Swift｜Testing System **None**（測試在 SPM package 內）｜Storage None｜Minimum Deployments **iOS 16.0**。

環境已確認：Xcode 26.1.1 / Swift 6.2.1 / iPhone 16 Pro 模擬器可用。
